run-api:
	docker rm -f pizzerie-api && docker build -t pizzerie-api -f docker/Dockerfile.api --no-cache . && docker run --name pizzerie-api -p 8080:8080 --env-file docker/api.env -d pizzerie-api
