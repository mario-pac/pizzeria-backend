...
const HomeSetting: React.FC<ScreenBaseProps<'HomeSetting'>> = ({
    ...
    const getData = useCallback(async () => {
        try {
            ...
            showToast('error', 'Erro ao buscar servidor cadastrado: '+ (error as Error).message);
        } finally {
            setLoading(false);
        }
    }, [])
    ...
    const handleSalvar = async () => {
        ...
        navigation.goBack()
            ...
            showToast('error', 'Erro ao salvar os dados: '+(error as Error).message);
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <S.Container>
                <Input label="Servidor + porta" value={form.watch('server')} placeholder="192.168.1.3:8080" onChangeText={(s) => form.setValue('server', s)}/>
                <Spacer height={20}/>
                    <Button
                    value="Salvar"
